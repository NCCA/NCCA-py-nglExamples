...
This demo uses QML to overlay a set of floating panels on top of a `QOpenGLWidget` to mimic user interfaces similar to Dear ImGui.
...
- Because the GL widget and the Qt Quick surface are two separate widgets
  ...
  to `PyNGLScene` beneath (empty overlay space), this contrasts with
...
- There is no mouse-drag camera; `LookAtWidget` is the only camera control.
  ...
  space really do get delivered to `PyNGLScene`) but currently inert as

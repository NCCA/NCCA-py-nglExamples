...
"""
Object selection and manipulation demo.

A small scene of SelectionObject-derived objects that can be picked with the
mouse (colour-ID picking) and transformed with Maya-style gizmos.
...
class MainWindow(QOpenGLWindow):
    def __init__(self, parent: object = None) -> None:
        super().__init__()
        self.setTitle("Selection and Manipulation")
    ...
    def pick(self, x: float, y: float):
        ...
        x, y are in device pixels with Qt's top-left origin.
        """
        self.makeCurrent()
        gl.glViewport(0, 0, self.window_width, self.window_height)
        ...
        for obj in self.objects:
            obj.draw_pick(global_tx, self.view, self.project)
        if self.selected_objects():
            self.manipulator.draw_pick(self.mode, global_tx, self.view, self.project)
        gl.glClearColor(0.4, 0.4, 0.4, 1.0)
        ...
        data = gl.glReadPixels(
            read_x, read_y, PICK_BLOCK, PICK_BLOCK, gl.GL_RGB, gl.GL_UNSIGNED_BYTE
        )

...
"""CurveDemos: samples a 4-point BezierCurve and draws the curve, control hull, and points."""
...
from ncca.ngl import BezierCurve, Mat4, Vec3, logger, look_at, perspective
...
class MainWindow(QOpenGLWindow):
    ...
    def initializeGL(self) -> None:
        ...
        self.curve_vao = self._create_polyline_vao(self.curve_points, gl.GL_LINE_STRIP)
        self.hull_vao = self._create_polyline_vao(
            self.curve.control_points, gl.GL_LINE_STRIP
        )
        self.points_vao = self._create_polyline_vao(
            self.curve.control_points, gl.GL_POINTS
        )

    @staticmethod
    def _create_polyline_vao(points: list[Vec3], mode: int):
        """Build a simple VAO holding a single vec3 position attribute (location 0)."""
        data = np.array([c for p in points for c in (p.x, p.y, p.z)], dtype=np.float32)
        vao = VAOFactory.create_vao(VAOType.SIMPLE, mode)
        with vao as v:
            v.set_data(VertexData(data, len(points)))
            v.set_vertex_attribute_pointer(0, 3, gl.GL_FLOAT, 3 * 4, 0)
            v.set_num_indices(len(points))
        return vao

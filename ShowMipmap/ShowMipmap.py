...
from ncca.ngl import Mat4, Transform, Vec3, logger, look_at, perspective
...
class MainWindow(QOpenGLWindow):
    ...
    def create_texture(self):
        ...
        # Using an image width of 128 gives us 6 mip levels: 128,64,32,16,8,4
        colours = [
            (1.0, 0.0, 0.0),  # red
            (0.0, 1.0, 0.0),  # green
            (0.0, 0.0, 1.0),  # blue
        ...
        while ml >= 4:
            print("mip level", ml)
            color = colours[mip_level]
            # create an ml x ml texture filled with `color`, dtype float32 to match GL_FLOAT
            data = np.full((ml, ml, 3), color, dtype=np.float32)

            print("data size", data.size, "using colour", color)

            # Set filters (set per-texture here; could also be set once after all levels uploaded)
            gl.glTexParameteri(
            ...
            # Note: internalFormat and format use GL_RGB and type GL_FLOAT to match the C++ code
            gl.glTexImage2D(
                gl.GL_TEXTURE_2D,
                mip_level,
                gl.GL_RGB,
        ...
        # Do not auto-generate mipmaps (we provided them manually)
        gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_BASE_LEVEL, 0)
        gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_MAX_LEVEL, mip_level - 1)
        gl.glTexParameteri(gl.GL_TEXTURE_2D, gl.GL_TEXTURE_MAX_LOD, mip_level - 1)
    ...
    def initializeGL(self) -> None:
        ...
        ShaderLib.load_shader(
            TEXTURE_SHADER, "shaders/TextureVert.glsl", "shaders/TextureFrag.glsl"
        )
        ...
        self._create_cube(0.2)
        self.create_texture()

    def _create_cube(self, scale) -> None:
    ...
    def paintGL(self) -> None:
        ...
        gl.glBindTexture(gl.GL_TEXTURE_2D, self.tex_id)
        gl.glBindVertexArray(self.vao_id)
        for z in np.linspace(-14, 15, 50):
            for x in np.linspace(-14, 15, 50):
                self.transform.reset()
                self.transform.set_position(x, 0, z)
                self.transform.set_rotation(x * 20.0, (x * z) * 40.0, z * 2.0)
                self.load_matrices_to_shader()

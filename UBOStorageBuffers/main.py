...
A second UBO, ``MaterialBlock { vec3 albedo; float shininess; }``, exists
to demonstrate the classic std140 trap: because a vec3 has a 16-byte base
alignment, ``shininess`` is NOT packed immediately after albedo's 3 floats
at byte offset 12 -- the compiler places it at offset 16. Key ``X`` swaps
the CPU-side numpy dtype used to build the upload buffer between the
correct std140 layout and a naive tightly-packed one; the shader itself
never changes, only the bytes fed to it -- so toggling X visibly corrupts
the teapot's specular highlight (shininess reads back as 0) without
...
class MainWindow(PySideEventHandlingMixin, QOpenGLWindow):
    ...
    def _draw_hud(self) -> None:
        ...
        Text.render_text(
            ...
            10,
            42,
            f"[X] MaterialBlock (binding 1) layout: {layout_name}",
            Vec3(1.0, 1.0, 0.6)
            if self.correct_material_layout

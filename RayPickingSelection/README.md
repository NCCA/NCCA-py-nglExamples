...
1. **Transform the ray into object-local space** with one 4x4 inverse
   (`transform_ray`). The direction is *not* re-normalised — because the
...
gizmos, and it makes the click tolerance a clean DPI-independent radius —
...
* **Con** — needs CPU-side triangle data, and very heavy meshes would want
  ...
  *rendered* silhouettes (e.g. alpha-tested cutouts) where geometry alone
  can't tell.

...
class MeshPreviewWidget(PySideEventHandlingMixin, QOpenGLWidget, MeshRenderMixin):
    ...
    def __init__(self, state: MeshRenderState, parent: object = None) -> None:
        ...
        # Mouse only. This preview sits inside a node body, so the mixin's own
        # shortcuts have no business here: Escape would close the whole editor,
        # and W, S and Space would change the polygon mode or throw the camera
        # away whenever the preview happened to hold the focus.
        self.setup_event_handling(
            initial_position=Vec3(0, 0, -2), handle_key_shortcuts=False
        )

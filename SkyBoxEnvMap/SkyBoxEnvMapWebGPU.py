#!/usr/bin/env -S uv run --script
"""
Cubemaps: skybox rendering plus reflection / refraction on a teapot (WebGPU).

Same scene and controls as the OpenGL version (main.py). The procedural sky
(cubemap_gen.py) is uploaded as one wgpu texture with 6 array layers,
viewed with dimension="cube" and sampled with texture_cube<f32> in WGSL.

    M    cycle teapot shading mode: reflect / refract / fresnel mix / diffuse
    +/-  increase / decrease the index of refraction (refract & fresnel mix)
    LMB  rotate    RMB pan    wheel zoom    Space reset camera    Esc quit

"""

import sys
from pathlib import Path

import numpy as np
import wgpu
from cubemap_gen import generate_cubemap_faces
from ncca.ngl import Mat4, PerspMode, PrimData, Prims, Vec3, look_at, perspective
from PySide6.QtCore import Qt, QTimer
from PySide6.QtGui import QColor
from PySide6.QtWidgets import QApplication
from WebGPUWidget import WebGPUWidget
from wgpu.utils import get_default_device

_FLOAT_SIZE = np.dtype(np.float32).itemsize

# (label, mode index) cycled with M
MODES = (
    ("reflect", 0),
    ("refract", 1),
    ("fresnel mix", 2),
    ("plain diffuse", 3),
)

_SKY_UNIFORM_DTYPE = np.dtype(
    [
        ("view_rot", np.float32, (4, 4)),
        ("proj", np.float32, (4, 4)),
    ]
)

_TRANSFORM_DTYPE = np.dtype(
    [
        ("mvp", np.float32, (4, 4)),
        ("model", np.float32, (4, 4)),
        ("normal_matrix", np.float32, (4, 4)),
    ]
)

_PARAMS_DTYPE = np.dtype(
    [
        ("cam_pos", np.float32, 4),
        ("light_dir", np.float32, 4),
        ("mode_ior", np.float32, 4),
    ]
)


class WebGPUScene(WebGPUWidget):
    """Skybox + reflective/refractive teapot scene."""

    def __init__(self):
        super().__init__()
        self.setWindowTitle("SkyBox / EnvMap (WebGPU)")
        self.msaa_sample_count = 4

        self.mode_index = 0
        self.ior = 1.52  # glass

        # --- camera / mouse state (same conventions as the GL demos) ---
        self.mouse_global_tx = Mat4()
        self.model_position = Vec3()
        self.rotate = False
        self.translate = False
        self.spin_x_face = 0
        self.spin_y_face = 0
        self.original_x_rotation = 0
        self.original_y_rotation = 0
        self.original_x_pos = 0
        self.original_y_pos = 0
        self.INCREMENT = 0.01
        self.ZOOM = 0.1

        self.eye = Vec3(0.0, 1.5, 6.0)
        self.view = look_at(self.eye, Vec3(0.0, 1.0, 0.0), Vec3(0.0, 1.0, 0.0))
        self.project = perspective(
            45.0, self.width() / self.height(), 0.1, 350.0, PerspMode.WebGPU
        )

        self.device = get_default_device()
        self._create_cube_texture()
        self._create_skybox_pipeline()
        self._create_envmap_pipeline()
        self._create_render_buffer()
        self.update()

    # ------------------------------------------------------------------
    # cubemap texture
    # ------------------------------------------------------------------
    def _create_cube_texture(self) -> None:
        faces = generate_cubemap_faces()
        size = faces[0].shape[0]
        self.cube_texture = self.device.create_texture(
            size=(size, size, 6),
            dimension=wgpu.TextureDimension.d2,
            format=wgpu.TextureFormat.rgba8unorm,
            usage=wgpu.TextureUsage.TEXTURE_BINDING | wgpu.TextureUsage.COPY_DST,
        )
        # FACE_ORDER (+x,-x,+y,-y,+z,-z) matches the GL cubemap face order,
        # and the array layer index doubles as the cube face index -- the
        # same order that GL uploads with GL_TEXTURE_CUBE_MAP_POSITIVE_X+i.
        for layer, face_pixels in enumerate(faces):
            self.device.queue.write_texture(
                {"texture": self.cube_texture, "mip_level": 0, "origin": (0, 0, layer)},
                np.ascontiguousarray(face_pixels).tobytes(),
                {"bytes_per_row": size * 4, "rows_per_image": size},
                (size, size, 1),
            )
        self.cube_texture_view = self.cube_texture.create_view(
            dimension=wgpu.TextureViewDimension.cube
        )
        self.cube_sampler = self.device.create_sampler(
            mag_filter=wgpu.FilterMode.linear,
            min_filter=wgpu.FilterMode.linear,
            address_mode_u=wgpu.AddressMode.clamp_to_edge,
            address_mode_v=wgpu.AddressMode.clamp_to_edge,
            address_mode_w=wgpu.AddressMode.clamp_to_edge,
        )

    # ------------------------------------------------------------------
    # pipelines
    # ------------------------------------------------------------------
    def _create_skybox_pipeline(self) -> None:
        shader_src = (Path(__file__).parent / "SkyBoxShader.wgsl").read_text()
        shader_module = self.device.create_shader_module(code=shader_src)

        cube_data = PrimData.primitive(Prims.CUBE.value)
        self.cube_vertex_count = cube_data.size // 8
        self.cube_vertex_buffer = self.device.create_buffer_with_data(
            data=cube_data, usage=wgpu.BufferUsage.VERTEX
        )

        bind_group_layout = self.device.create_bind_group_layout(
            entries=[
                {
                    "binding": 0,
                    "visibility": wgpu.ShaderStage.VERTEX,
                    "buffer": {"type": wgpu.BufferBindingType.uniform},
                },
                {
                    "binding": 1,
                    "visibility": wgpu.ShaderStage.FRAGMENT,
                    "texture": {"view_dimension": wgpu.TextureViewDimension.cube},
                },
                {
                    "binding": 2,
                    "visibility": wgpu.ShaderStage.FRAGMENT,
                    "sampler": {"type": wgpu.SamplerBindingType.filtering},
                },
            ]
        )
        pipeline_layout = self.device.create_pipeline_layout(
            bind_group_layouts=[bind_group_layout]
        )

        self.sky_uniforms = np.zeros((), dtype=_SKY_UNIFORM_DTYPE)
        self.sky_uniform_buffer = self.device.create_buffer(
            size=self.sky_uniforms.nbytes,
            usage=wgpu.BufferUsage.UNIFORM | wgpu.BufferUsage.COPY_DST,
        )
        self.sky_bind_group = self.device.create_bind_group(
            layout=bind_group_layout,
            entries=[
                {
                    "binding": 0,
                    "resource": {
                        "buffer": self.sky_uniform_buffer,
                        "offset": 0,
                        "size": self.sky_uniform_buffer.size,
                    },
                },
                {"binding": 1, "resource": self.cube_texture_view},
                {"binding": 2, "resource": self.cube_sampler},
            ],
        )

        self.sky_pipeline = self.device.create_render_pipeline(
            label="skybox_pipeline",
            layout=pipeline_layout,
            vertex={
                "module": shader_module,
                "entry_point": "vertex_main",
                "buffers": [
                    {
                        "array_stride": 8 * _FLOAT_SIZE,
                        "step_mode": "vertex",
                        "attributes": [
                            {"format": "float32x3", "offset": 0, "shader_location": 0},
                        ],
                    }
                ],
            },
            fragment={
                "module": shader_module,
                "entry_point": "fragment_main",
                "targets": [{"format": wgpu.TextureFormat.rgba8unorm}],
            },
            primitive={"topology": wgpu.PrimitiveTopology.triangle_list},
            # drawn last, LEQUAL + no depth write: only wins where nothing
            # opaque has already written a closer depth (see module docstring)
            depth_stencil={
                "format": wgpu.TextureFormat.depth24plus,
                "depth_write_enabled": False,
                "depth_compare": wgpu.CompareFunction.less_equal,
            },
            multisample={
                "count": self.msaa_sample_count,
                "mask": 0xFFFFFFFF,
                "alpha_to_coverage_enabled": False,
            },
        )

    def _create_envmap_pipeline(self) -> None:
        shader_src = (Path(__file__).parent / "EnvMapShader.wgsl").read_text()
        shader_module = self.device.create_shader_module(code=shader_src)

        teapot_data = PrimData.primitive(Prims.TEAPOT.value)
        self.teapot_vertex_count = teapot_data.size // 8
        self.teapot_vertex_buffer = self.device.create_buffer_with_data(
            data=teapot_data, usage=wgpu.BufferUsage.VERTEX
        )

        bind_group_layout = self.device.create_bind_group_layout(
            entries=[
                {
                    "binding": 0,
                    "visibility": wgpu.ShaderStage.VERTEX,
                    "buffer": {"type": wgpu.BufferBindingType.uniform},
                },
                {
                    "binding": 1,
                    "visibility": wgpu.ShaderStage.FRAGMENT,
                    "buffer": {"type": wgpu.BufferBindingType.uniform},
                },
                {
                    "binding": 2,
                    "visibility": wgpu.ShaderStage.FRAGMENT,
                    "texture": {"view_dimension": wgpu.TextureViewDimension.cube},
                },
                {
                    "binding": 3,
                    "visibility": wgpu.ShaderStage.FRAGMENT,
                    "sampler": {"type": wgpu.SamplerBindingType.filtering},
                },
            ]
        )
        pipeline_layout = self.device.create_pipeline_layout(
            bind_group_layouts=[bind_group_layout]
        )

        self.transform_uniforms = np.zeros((), dtype=_TRANSFORM_DTYPE)
        self.transform_buffer = self.device.create_buffer(
            size=self.transform_uniforms.nbytes,
            usage=wgpu.BufferUsage.UNIFORM | wgpu.BufferUsage.COPY_DST,
        )
        self.params_uniforms = np.zeros((), dtype=_PARAMS_DTYPE)
        self.params_buffer = self.device.create_buffer(
            size=self.params_uniforms.nbytes,
            usage=wgpu.BufferUsage.UNIFORM | wgpu.BufferUsage.COPY_DST,
        )
        self.envmap_bind_group = self.device.create_bind_group(
            layout=bind_group_layout,
            entries=[
                {
                    "binding": 0,
                    "resource": {
                        "buffer": self.transform_buffer,
                        "offset": 0,
                        "size": self.transform_buffer.size,
                    },
                },
                {
                    "binding": 1,
                    "resource": {
                        "buffer": self.params_buffer,
                        "offset": 0,
                        "size": self.params_buffer.size,
                    },
                },
                {"binding": 2, "resource": self.cube_texture_view},
                {"binding": 3, "resource": self.cube_sampler},
            ],
        )

        self.envmap_pipeline = self.device.create_render_pipeline(
            label="envmap_pipeline",
            layout=pipeline_layout,
            vertex={
                "module": shader_module,
                "entry_point": "vertex_main",
                "buffers": [
                    {
                        "array_stride": 8 * _FLOAT_SIZE,
                        "step_mode": "vertex",
                        "attributes": [
                            {"format": "float32x3", "offset": 0, "shader_location": 0},
                            {"format": "float32x3", "offset": 12, "shader_location": 1},
                            {"format": "float32x2", "offset": 24, "shader_location": 2},
                        ],
                    }
                ],
            },
            fragment={
                "module": shader_module,
                "entry_point": "fragment_main",
                "targets": [{"format": wgpu.TextureFormat.rgba8unorm}],
            },
            primitive={"topology": wgpu.PrimitiveTopology.triangle_list},
            depth_stencil={
                "format": wgpu.TextureFormat.depth24plus,
                "depth_write_enabled": True,
                "depth_compare": wgpu.CompareFunction.less,
            },
            multisample={
                "count": self.msaa_sample_count,
                "mask": 0xFFFFFFFF,
                "alpha_to_coverage_enabled": False,
            },
        )

    # ------------------------------------------------------------------
    # scene helpers
    # ------------------------------------------------------------------
    def scene_global_tx(self) -> Mat4:
        rot_x = Mat4().rotate_x(self.spin_x_face)
        rot_y = Mat4().rotate_y(self.spin_y_face)
        tx = rot_y @ rot_x
        tx[3, 0] = self.model_position.x
        tx[3, 1] = self.model_position.y
        tx[3, 2] = self.model_position.z
        return tx

    # ------------------------------------------------------------------
    # rendering
    # ------------------------------------------------------------------
    def paintWebGPU(self) -> None:
        global_tx = self.scene_global_tx()

        model_view = self.view @ global_tx
        self.transform_uniforms["mvp"] = (self.project @ model_view).to_numpy()
        self.transform_uniforms["model"] = global_tx.to_numpy()
        self.transform_uniforms["normal_matrix"] = (
            global_tx.inverse().transposed().to_numpy()
        )
        self.device.queue.write_buffer(
            self.transform_buffer, 0, self.transform_uniforms.tobytes()
        )

        self.params_uniforms["cam_pos"] = (*self.eye.to_numpy(), 1.0)
        self.params_uniforms["light_dir"] = (0.4, 1.0, 0.6, 0.0)
        self.params_uniforms["mode_ior"] = (
            float(MODES[self.mode_index][1]),
            self.ior,
            0.0,
            0.0,
        )
        self.device.queue.write_buffer(
            self.params_buffer, 0, self.params_uniforms.tobytes()
        )

        view_rotation_only = self.view.copy()
        view_rotation_only[3, 0] = 0.0
        view_rotation_only[3, 1] = 0.0
        view_rotation_only[3, 2] = 0.0
        self.sky_uniforms["view_rot"] = view_rotation_only.to_numpy()
        self.sky_uniforms["proj"] = self.project.to_numpy()
        self.device.queue.write_buffer(
            self.sky_uniform_buffer, 0, self.sky_uniforms.tobytes()
        )

        command_encoder = self.device.create_command_encoder()
        render_pass = command_encoder.begin_render_pass(
            color_attachments=[
                {
                    "view": self.multisample_texture_view,
                    "resolve_target": self.colour_buffer_texture_view,
                    "load_op": wgpu.LoadOp.clear,
                    "store_op": wgpu.StoreOp.store,
                    "clear_value": (0.1, 0.1, 0.12, 1.0),
                }
            ],
            depth_stencil_attachment={
                "view": self.depth_buffer_view,
                "depth_load_op": wgpu.LoadOp.clear,
                "depth_store_op": wgpu.StoreOp.store,
                "depth_clear_value": 1.0,
            },
        )

        # 1. opaque pass: env-mapped teapot, depth write on
        render_pass.set_pipeline(self.envmap_pipeline)
        render_pass.set_bind_group(0, self.envmap_bind_group, [], 0, 999999)
        render_pass.set_vertex_buffer(0, self.teapot_vertex_buffer)
        render_pass.draw(self.teapot_vertex_count)

        # 2. skybox pass, drawn LAST so early-z rejects fragments behind
        # already-shaded opaque geometry
        render_pass.set_pipeline(self.sky_pipeline)
        render_pass.set_bind_group(0, self.sky_bind_group, [], 0, 999999)
        render_pass.set_vertex_buffer(0, self.cube_vertex_buffer)
        render_pass.draw(self.cube_vertex_count)

        render_pass.end()
        self.device.queue.submit([command_encoder.finish()])
        self._update_colour_buffer()
        self._draw_hud()

    def _draw_hud(self) -> None:
        white = QColor(255, 255, 255)
        label, _ = MODES[self.mode_index]
        state = f"[M] mode: {label}   [+/-] IOR: {self.ior:.2f}"
        self.render_text(10, 20, state, 14, "Arial", white)

    def resizeWebGPU(self, width, height) -> None:
        self.project = perspective(45.0, width / height, 0.1, 350.0, PerspMode.WebGPU)
        self.update()

    # ------------------------------------------------------------------
    # input
    # ------------------------------------------------------------------
    def keyPressEvent(self, event) -> None:
        key = event.key()
        if key == Qt.Key_Escape:
            self.close()
        elif key == Qt.Key_M:
            self.mode_index = (self.mode_index + 1) % len(MODES)
        elif key in (Qt.Key_Plus, Qt.Key_Equal):
            self.ior = min(2.5, self.ior + 0.05)
        elif key in (Qt.Key_Minus, Qt.Key_Underscore):
            self.ior = max(1.01, self.ior - 0.05)
        elif key == Qt.Key_Space:
            self.spin_x_face = 0
            self.spin_y_face = 0
            self.model_position.set(0, 0, 0)
        self.update()
        super().keyPressEvent(event)

    def mousePressEvent(self, event) -> None:
        position = event.position()
        if event.button() == Qt.LeftButton:
            self.original_x_rotation = position.x()
            self.original_y_rotation = position.y()
            self.rotate = True
        elif event.button() == Qt.RightButton:
            self.original_x_pos = position.x()
            self.original_y_pos = position.y()
            self.translate = True

    def mouseMoveEvent(self, event) -> None:
        if self.rotate and event.buttons() == Qt.LeftButton:
            position = event.position()
            diff_x = position.x() - self.original_x_rotation
            diff_y = position.y() - self.original_y_rotation
            self.spin_x_face += int(0.5 * diff_y)
            self.spin_y_face += int(0.5 * diff_x)
            self.original_x_rotation = position.x()
            self.original_y_rotation = position.y()
            self.update()
        elif self.translate and event.buttons() == Qt.RightButton:
            position = event.position()
            diff_x = int(position.x() - self.original_x_pos)
            diff_y = int(position.y() - self.original_y_pos)
            self.original_x_pos = position.x()
            self.original_y_pos = position.y()
            self.model_position.x += self.INCREMENT * diff_x
            self.model_position.y -= self.INCREMENT * diff_y
            self.update()

    def mouseReleaseEvent(self, event) -> None:
        if event.button() == Qt.LeftButton:
            self.rotate = False
        elif event.button() == Qt.RightButton:
            self.translate = False

    def wheelEvent(self, event) -> None:
        delta = event.angleDelta().y()
        self.model_position.z += self.ZOOM * (delta / 120.0)
        self.update()


def main():
    app = QApplication(sys.argv)
    win = WebGPUScene()
    win.resize(1024, 720)
    win.show()

    if "--smoketest" in sys.argv:
        QTimer.singleShot(200, lambda: (print("SMOKETEST OK"), app.quit()))

    sys.exit(app.exec())


if __name__ == "__main__":
    main()
